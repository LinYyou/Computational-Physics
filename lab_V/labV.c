...
int main(int argc,char* argv[]){
  double v0, x0, dt, tTot, OmegaQ, xn, gamma, f0, OmegaEs, t, tau, tauBar, tStar;
  int n=1, i=0;
  FILE *fPos, *fNeg;
  ...
  */
  ...
  fPos = fopen("velPos.dat", "w");
  fNeg = fopen("velNeg.dat", "w");
  ...
  if(Metod == 4){
    ...
    for(t=0;t<tTot;t+=dt){
       ...
       if(point.x > M_PI || point.x < -M_PI) {
	 point.x -= (point.x/fabs(point.x))*2*M_PI;
       }
  ...
  */
  if(Metod == 4){
    for(i = 0; i<=1000; i++){
      ...
      for(t=0;t<tTot;t+=dt){
	point = RungeKutta4(point, dt, t, OmegaQ, gamma, f0, OmegaEs);
	if(t == tStar && point.v > 0) fprintf(fPos, "\n%lf    %lf    %lf", t, x0, v0);
	if(t == tStar && point.v < 0) fprintf(fNeg, "\n%lf    %lf    %lf", t, x0, v0);
